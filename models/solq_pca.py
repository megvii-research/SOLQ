...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
                 aux_loss=True, with_box_refine=False, two_stage=False, with_vector=False, processor_pca=None, vector_hidden_dim=256):
        ...
        self.processor_pca = processor_pca
        ...
        if self.with_vector:
            ...
            self.vector_embed = MLP(hidden_dim, vector_hidden_dim, self.processor_pca.dim_mask, 3)
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        src_vectors = outputs["pred_vectors"]
        src_boxes = outputs['pred_boxes']
        # TODO use valid to mask invalid areas due to padding in loss
        target_boxes = torch.cat([t['xyxy_boxes'][i] for t, (_, i) in zip(targets, indices)], dim=0)
        target_masks, valid = nested_tensor_from_tensor_list([t["masks"] for t in targets]).decompose()
        target_masks = target_masks.to(src_vectors)
        src_vectors = src_vectors[src_idx]
        src_boxes = src_boxes[src_idx]
        target_masks = target_masks[tgt_idx]

        # crop gt_masks
        n_keep, gt_mask_len = self.processor_pca.dim_mask, self.processor_pca.mask_size
        gt_masks = BitMasks(target_masks)
        gt_masks = gt_masks.crop_and_resize(target_boxes, gt_mask_len).to(device=src_vectors.device).float()
        ...
        target_vectors = self.processor_pca.encoder(target_masks.flatten(1))
        loss = mask_loss_func(src_vectors, target_vectors)
        
        losses = {}
        loss_feat = loss.sum() / num_boxes / self.processor_pca.dim_mask
        eps = 1e-5
        src_masks = self.processor_pca.decoder(src_vectors.flatten(1))
        ...
        loss_dice = loss.sum() / num_boxes
        losses["loss_vector"] = self.vector_loss_coef * (loss_feat + loss_dice)
        return losses
...
class PostProcess(nn.Module):
    ...
    def __init__(self, processor_pca=None):
        super().__init__()
        self.processor_pca = processor_pca
    ...
    def forward(self, outputs, target_sizes):
        ...
        if self.processor_pca is not None:
            n_keep = self.processor_pca.dim_mask
        ...
        if self.processor_pca is not None:
            ...
            b, r, c = vectors.shape
            for bi in range(b):
                vectors_per_image = vectors[bi].view(-1, n_keep)
                outputs_masks_per_image = self.processor_pca.decoder(vectors_per_image, is_train=False)
                outputs_masks_per_image = outputs_masks_per_image.view(-1, gt_mask_len, gt_mask_len).float().to(out_vector.device)
...
class PostProcessSegm(nn.Module):
    def __init__(self, threshold=0.5, processor_pca=None):
        super().__init__()
...
def build(args):
    ...
    if args.with_vector:
        processor_pca = ProcessorPCA(args.n_keep, args.gt_mask_len)
        # encoding parameters.
        """
        .npz file can be generate by 'tools/mask_generation.py'
        can be download from:
            https://drive.google.com/file/d/1LTeuakILAIchgNCBeTsVxovUeh8LCsAc/view?usp=sharing
        """
        components_path = '/data/dongbin/projects/PCA/coco_2017_train_class_agnosticTrue_whitenTrue_sigmoidTrue_60_siz28.npz'
        # update parameters.
        parameters = np.load(components_path)
        components = nn.Parameter(torch.from_numpy(parameters['components_c'][0]).float().to(device), requires_grad=False)
        explained_variances = nn.Parameter(torch.from_numpy(parameters['explained_variance_c'][0]).float().to(device), requires_grad=False)
        means = nn.Parameter(torch.from_numpy(parameters['mean_c'][0]).float().to(device), requires_grad=False)
        processor_pca.components = components
        processor_pca.explained_variances = explained_variances
        processor_pca.means = means
    ...
    model = DeformableDETR(
        ...
        processor_pca=processor_pca if args.with_vector else None,
    ...
    criterion.to(device)
    postprocessors = {'bbox': PostProcess(processor_pca=processor_pca if args.with_vector else None)}

    if args.masks:
        postprocessors['segm'] = PostProcessSegm(processor_pca=processor_pca if args.with_vector else None)
        if args.dataset_file == "coco_panoptic":
