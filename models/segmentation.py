...
class DeformableDETRsegm(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, seg_memory, seg_mask = self.detr.transformer(srcs, masks, pos, query_embeds)
        ...
        seg_masks = self.mask_head(srcs[-1], bbox_mask, [self.seg_input_proj[0](features[2].tensors), self.seg_input_proj[1](features[1].tensors), self.seg_input_proj[2](features[0].tensors)])
        outputs_seg_masks = seg_masks.view(bs, self.detr.num_queries, seg_masks.shape[-2], seg_masks.shape[-1])
