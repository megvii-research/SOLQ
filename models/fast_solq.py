...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        target_vectors = self.processor_dct.encode(target_masks)
        target_vectors = target_vectors.to(device=src_vectors.device)

        losses = {}
        mask_loss = F.l1_loss(src_vectors, target_vectors, reduction='none')
        mask_loss = self.vector_loss_coef * mask_loss / num_boxes
        mask_loss = torch.sum(mask_loss)
        losses['loss_vector'] = mask_loss
        return losses
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        if self.processor_dct is not None:
            ...
            b, r, c = vectors.shape
            for bi in range(b):
                outputs_masks_per_image = self.processor_dct.decode(vectors[bi].detach()).to(out_vector.device)
